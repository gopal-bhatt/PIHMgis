...
int catchment_shape(char *catFile, char *nodeFile, char *shpFile, char *dbfFile)
{
    ...
    if(polyFld == -1 || wshedID == -1)
    {
        printf("Unable to Add Field to DBF file\n");
        return 59;
    }
	int recordNum=0;
    ...
    if ( gridread(catFile,(void ***)&elev,RPFLTDTYPE,&nx,&ny,&dx,&dy,bndbox,&csize,&mval,&filetype) != 0)
    {
        printf("Unable to Read Catchment Grid File\n");
        return 80;
    }
    ...
    */
    ...
    for(i=0; i<numLine; i++){
       ...
       // printf("%d\n", i); getchar();getchar();
        ...
        if ( ShpEntryNumber < 0 )
        {
            printf("Unable to Add Shape Entry to SHP/SHX file\n");
            return 191;
        }
        //printf("%d\n", i); getchar();getchar();
        ...
        {
            printf("Unable to Write Integer Attribute to DBF file\n");
            return 199;
        }
        temprecordNum++;
        //printf("%d\n", i); getchar(); getchar();
