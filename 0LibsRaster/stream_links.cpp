...
void streamGen(float **riv, short **fdr, int i, int j, int*);
...
int stream_links(char *rivFile, char *fdrFile, char *segFile, char *nodeFile){
    ...
    fclose(fpNode_Seg);
    err4 = gridwrite(segFile,(void **)slope,RPFLTDTYPE,nx,ny,dx,dy,bndbox,csize,mval,filetype);
    err = err1 + err2 + err3 + err4;
    return(err);
}

// Recursively generates the river segments starting from outlet(i,j)
void streamGen(float **riv, short **fdr, int i, int j, int *num){
//    static int num = 1;
    int stop = 0;
    int ii;
    int cell[MAX][2];
         ...
         if(temp==0){
             ...
             if(temp>1){
                 for(ii=0; ii<temp; ii++){
                     ...
                     streamGen(riv, fdr, cell[ii][0], cell[ii][1],num);
                 }
             }
         }
    }
